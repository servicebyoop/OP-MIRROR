...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[START:], start=1):
            msg += f"<b>╭─🗂️ Fɪʟᴇɴᴀᴍᴇ:</b> <code>{escape(str(download.name()))}</code>"
         
            msg += f"\n<b>├─⚙️ Sᴛᴀᴛᴜꜱ:</b> <i>{download.status()}</i>"
            if download.status() not in [
                ...
                msg += f"\n<b>├─🕴️ User :</b> {get_readable_user_name}
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    ...
                    msg += f"\n<b>├─🖥️ Dᴏᴡɴʟᴏᴀᴅᴇᴅ:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                msg += f"\n<b>⏱️ Speed:</b> {download.speed()} | <b>⌛Eᴛᴀ:</b> {download.eta()}"
                try:
        ...
        bmsg = f"<b>💻 Cᴘᴜ:</b> {cpu_percent()}% | <b>🗑️ Fʀᴇᴇ:</b> {free}"
